...
async function handler(req, res) {        //req object contains data about the incoming request(either a post or get request) and the res object will be needed for sending back a response for the request sent
  ...
  if (req.method === 'POST') {
    ...
    //establish a connection to a created database in the cluster, if such database isnt already created/available then newly create one (themeetups database) and connect on the go
    ...
    //insert the requestData obj (i.e a document) into the collection. This returns a promise which is a result of insertion(e.g an object with an automatically id)
